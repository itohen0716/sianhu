(function(global){
  ...
  function render(state,options={}){
    ...
    const vocalPositionById=new Map(),shamisenPositionByAnchor=new Map();rows.forEach(row=>{row.totalWidth=row.measures.reduce((sum,measure)=>sum+measure.width,0);row.vocalSlurs=[];row.tuplets=[];let used=0;row.measures.forEach(measure=>{measure.vocalNotes.forEach(note=>vocalPositionById.set(String(note.id),{row:row.row,x:(used+note.left*measure.width)/row.totalWidth*1000}));measure.notes.filter(note=>!note.rest).forEach(note=>{shamisenPositionByAnchor.set(`${measure.m},${Number(note.s)},${Number(note.p)}`,{row:row.row,x:(used+note.left*measure.width)/row.totalWidth*1000,y:23+(3-Number(note.s))*18})});used+=measure.width})});
