(function(global){
  ...
  function render(state,options={}){
    ...
    const annotationCapture=captureAnnotations(state),geometry=captureScoreGeometry(),rows=counts.map((count,row)=>({row,measures:Array.from({length:count},(_,local)=>{const m=starts[row]+local,width=measureWidth(m),special=state.measureSpecials?.[String(m)];return{m,local,width,hanma:special?.type==="hanma"&&Number(special.lengthRatio)===0.5,startKind:state.barlineKinds?.[barKey(row,local)]||"single",endKind:local===count-1?(state.barlineKinds?.[barKey(row,count)]||"single"):"single",notes:(state.notes||[]).filter(note=>note.m===m).map(note=>{const x=note.rest?(Number(note.x)||0):columnX(note.m,note.p),id=key(note.m,note.s,note.p),measured=geometry.noteLeft.get(id);return{...note,left:Number.isFinite(measured)?measured:clamp((Number(note.p)+.5)/capacity+x/width,.005,.995),value:note.rest?"●":(isOpenValue(note.v)?"0":String(note.v||"")),techniques:note.rest?[]:(techMap.get(id)||[])} }),vocalNotes:(state.vocalNotes||[]).filter(note=>Number(note.m)===m).map(note=>{const anchor=note.anchor&&Number(note.anchor.m)===m&&Number.isInteger(Number(note.anchor.p))?note.anchor:null,position=anchor?Number(anchor.p):Number(note.p),offset=anchor?Math.max(0,Number(note.anchorOffset)||0):0,measured=anchor?geometry.columnLeft.get(`${m},${position}`):null,base=Number.isFinite(measured)?measured:(position+.5)/capacity+(anchor?columnX(m,position)/width:0);return{...note,left:clamp(base+offset/capacity+(Number(note.x)||0)/width,.005,.995)}}),parts:(state.scoreParts||[]).filter(part=>part.m===m).map(part=>{const measured=geometry.partLeft.get(String(part.id||""));return{...part,left:Number.isFinite(measured)?measured:clamp((Number(part.p)+.5)/capacity+(Number(part.x)||0)/width,.005,.995)}})}})}));
    const vocalPositionById=new Map(),shamisenPositionByColumn=new Map();rows.forEach(row=>{row.totalWidth=row.measures.reduce((sum,measure)=>sum+measure.width,0);row.vocalSlurs=[];row.tuplets=[];let used=0;row.measures.forEach(measure=>{measure.vocalNotes.forEach(note=>vocalPositionById.set(String(note.id),{row:row.row,x:(used+note.left*measure.width)/row.totalWidth*1000}));const positions=new Map();measure.notes.filter(note=>!note.rest).forEach(note=>{const list=positions.get(Number(note.p))||[];list.push(note.left);positions.set(Number(note.p),list)});positions.forEach((values,p)=>{const left=values.reduce((sum,value)=>sum+value,0)/values.length;shamisenPositionByColumn.set(`${measure.m},${p}`,{row:row.row,x:(used+left*measure.width)/row.totalWidth*1000})});used+=measure.width})});
    (state.tuplets||[]).forEach(tuplet=>{let start=shamisenPositionByColumn.get(`${Number(tuplet.start?.m)},${Number(tuplet.start?.p)}`),end=shamisenPositionByColumn.get(`${Number(tuplet.end?.m)},${Number(tuplet.end?.p)}`);if(!start||!end||start.row!==end.row)return;if(start.x>end.x)[start,end]=[end,start];if(end.x-start.x>=8)rows[start.row]?.tuplets.push({start:start.x,end:end.x})});
