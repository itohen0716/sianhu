(function(global){
  ...
  function render(state,options={}){
    ...
    root.innerHTML=pages.map(page=>`<section class="pv2-page">${page.index===0?`<div class="pv2-meta"><div class="pv2-meta-left"><span>〈</span><span>${esc(state.tuning||"")}</span><span>〉</span><span>${esc(meterLabel())}</span></div><div class="pv2-title">${esc(state.title||"曲名入力")}</div><div></div></div>`:""}${page.rows.map(row=>`<section class="pv2-staff" style="height:${((283*96/25.4-(page.index===0?34:0))/page.rows.length).toFixed(2)}px"><div class="pv2-wrap"><div class="pv2-labels">${row.row===0?"<span>三の糸</span><span>二の糸</span><span>一の糸</span>":"<span></span><span></span><span></span>"}</div><div class="pv2-score">${row.measures.map(measureHtml).join("")}${row.annotations.length?`<svg class="pv2-overlay" viewBox="0 -60 1000 180" preserveAspectRatio="none">${row.annotations.map(annotationHtml).join("")}</svg>`:""}</div></div></section>`).join("")}</section>`).join("");
