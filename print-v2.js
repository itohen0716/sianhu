(function(global){
  ...
  function render(state,options={}){
    ...
    const techniqueLayer=(state.layers||[]).find(layer=>layer.id==="technique-layer"),techMap=new Map(),linkedTechniqueSlurs=[];
    if(techniqueLayer?.visible!==false)(techniqueLayer?.items||[]).filter(item=>item.type==="symbol"&&item.anchor).forEach(item=>{if(item.text==="スリ"&&item.endAnchor){linkedTechniqueSlurs.push(item);return}const k=key(item.anchor.m,item.anchor.s,item.anchor.p);if(!techMap.has(k))techMap.set(k,[]);techMap.get(k).push(item)});
    ...
    const printScoreWidth=196*96/25.4-55,printLyricsWidth=printScoreWidth,vocalPositionById=new Map(),shamisenPositionByAnchor=new Map();rows.forEach(row=>{row.totalWidth=row.measures.reduce((sum,measure)=>sum+measure.width,0);row.vocalSlurs=[];row.tuplets=[];row.techniqueSlurs=[];let used=0;row.measures.forEach(measure=>{measure.vocalNotes.forEach(note=>vocalPositionById.set(String(note.id),{row:row.row,x:(used+note.left*measure.width)/row.totalWidth*1000}));measure.notes.filter(note=>!note.rest).forEach(note=>{shamisenPositionByAnchor.set(`${measure.m},${Number(note.s)},${Number(note.p)}`,{row:row.row,x:(used+note.left*measure.width)/row.totalWidth*1000,y:23+(3-Number(note.s))*18})});used+=measure.width})});
    ...
    rows.forEach(item=>{const sourceWidth=geometry.lyricsWidth.get(item.row)||1000;item.lyrics=showLyrics?String(state.lyrics?.[item.row]||""):"";item.lyricsSourceWidth=sourceWidth;item.lyricsScale=printLyricsWidth/sourceWidth});
    ...
    root.dataset.modelVersion="9";root.dataset.noteCount=String((state.notes||[]).length);root.dataset.annotationCount=String(mappedAnnotations.length);
  }
