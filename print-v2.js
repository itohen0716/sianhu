(function(global){
  ...
  function render(state,options={}){
    ...
    const overlayTopOffset=12*96/25.4+(state.lyricsMode?20:24);
    ...
    const vocalMark=stringNumber=>Number(stringNumber)===1?"••":Number(stringNumber)===2?"•":"",vocalNoteHtml=note=>`<span class="pv2-vocal-note d${[1,2,4].includes(Number(note.d))?Number(note.d):4}" style="left:${(note.left*100).toFixed(5)}%"><span class="pv2-vocal-value">${esc(note.v)}</span><span class="pv2-vocal-mark">${note.kind==="rest"||note.kind==="interjection"?"":vocalMark(note.s)}</span></span>`;
