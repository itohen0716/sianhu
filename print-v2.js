(function(global){
  ...
  function render(state,options={}){
    ...
    rows.forEach(row=>{const totalWidth=row.measures.reduce((sum,measure)=>sum+measure.width,0),positionById=new Map(),prefix=[];let used=0;row.measures.forEach(measure=>{prefix.push(used);measure.vocalNotes.forEach(note=>positionById.set(String(note.id),used+note.left*measure.width));used+=measure.width});row.vocalSlurs=(state.vocalSlurs||[]).flatMap(slur=>{const start=positionById.get(String(slur.startId)),end=positionById.get(String(slur.endId));return Number.isFinite(start)&&Number.isFinite(end)&&start!==end?[{start:Math.min(start,end)/totalWidth*1000,end:Math.max(start,end)/totalWidth*1000}]:[]})});
