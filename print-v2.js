(function(global){
  ...
  function render(state,options={}){
    ...
    root.innerHTML=pages.map(page=>{const firstRow=page.rows[0]?.row||0,lastRow=firstRow+page.rows.length,pageAnnotations=mappedAnnotations.filter(item=>item.annotationRow>=firstRow&&item.annotationRow<lastRow).map(item=>({...item,pointsNormalized:item.pointsNormalized.map(point=>({...point,y:point.y-firstRow*sharedStaffHeight}))})),overlayHeight=overlayTopOffset+page.rows.length*sharedStaffHeight;return`<section class="pv2-page">${page.index===0?`<div class="pv2-meta"><div class="pv2-meta-left" aria-label="調子と拍子"><span>〈${esc(state.tuning||"")}〉</span><span>${esc(meterLabel())}</span></div><div class="pv2-title">${esc(state.title||"曲名入力")}</div></div>`:""}<div class="pv2-staffs${state.lyricsMode?" lyrics-mode":""}">${page.rows.map(row=>`<section class="pv2-staff${row.row===0?" first-score-row":""}" style="height:${sharedStaffHeight.toFixed(2)}px"><div class="pv2-wrap"><div class="pv2-labels">${row.row===0?"<span>三の糸</span><span>二の糸</span><span>一の糸</span>":"<span></span><span></span><span></span>"}</div><div class="pv2-score">${row.measures.map(measureHtml).join("")}</div></div>${state.lyricsMode?`<div class="pv2-lyrics"><span class="pv2-lyrics-track" style="width:${row.lyricsSourceWidth.toFixed(2)}px;transform:scaleX(${row.lyricsScale.toFixed(6)})">${esc(row.lyrics)}</span></div>`:""}</section>`).join("")}${pageAnnotations.length?`<svg class="pv2-page-overlay" style="height:${overlayHeight.toFixed(2)}px" viewBox="0 0 1000 ${overlayHeight.toFixed(2)}" preserveAspectRatio="none">${pageAnnotations.map(annotationHtml).join("")}</svg>`:""}</div></section>`}).join("");
