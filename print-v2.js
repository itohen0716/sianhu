(function(global){
  ...
  function barSvg(kind,end=false){
    if(kind==="single")return"";
    if(end){if(kind==="double")return`<svg viewBox="0 0 18 68">${line(11)}${line(17)}</svg>`;if(kind==="repeat-end")return`<svg viewBox="0 0 18 68">${dots(4)}${line(11)}${line(17)}</svg>`;if(kind==="final")return`<svg viewBox="0 0 18 68">${line(10)}${line(16,4)}</svg>`}
  ...
  function render(state,options={}){
    ...
    const measureHtml=measure=>`<div class="pv2-measure" style="--grow:${measure.width}"><span class="pv2-boundary start${measure.startKind!=="single"?" custom":""}">${measure.startKind==="single"?"":barSvg(measure.startKind)}</span>${measure.endKind!=="single"?`<span class="pv2-boundary end custom">${barSvg(measure.endKind,true)}</span>`:""}${measure.parts.map(part=>`<span class="pv2-part" style="left:${(part.left*100).toFixed(5)}%">${sameSvg(part.kind)}</span>`).join("")}${[3,2,1].map(string=>`<div class="pv2-string">${measure.notes.filter(note=>Number(note.s)===string).map(noteHtml).join("")}</div>`).join("")}</div>`;
