(function(global){
  ...
  function render(state,options={}){
    ...
    const annotationCapture=captureAnnotations(state),geometry=captureScoreGeometry(),rows=counts.map((count,row)=>({row,measures:Array.from({length:count},(_,local)=>{const m=starts[row]+local,width=measureWidth(m);return{m,local,width,startKind:state.barlineKinds?.[barKey(row,local)]||"single",endKind:local===count-1?(state.barlineKinds?.[barKey(row,count)]||"single"):"single",notes:(state.notes||[]).filter(note=>note.m===m).map(note=>{const x=note.rest?(Number(note.x)||0):columnX(note.m,note.p),id=key(note.m,note.s,note.p),measured=geometry.noteLeft.get(id);return{...note,left:Number.isFinite(measured)?measured:clamp((Number(note.p)+.5)/capacity+x/width,.005,.995),value:note.rest?"●":String(note.v||""),techniques:note.rest?[]:(techMap.get(id)||[])} }),vocalNotes:(state.vocalNotes||[]).filter(note=>Number(note.m)===m).map(note=>{const anchor=note.anchor&&Number(note.anchor.m)===m&&Number.isInteger(Number(note.anchor.p))?note.anchor:null,position=anchor?Number(anchor.p):Number(note.p),anchorX=anchor?columnX(m,position):0;return{...note,left:clamp((position+.5)/capacity+(anchorX+(Number(note.x)||0))/width,.005,.995)}}),parts:(state.scoreParts||[]).filter(part=>part.m===m).map(part=>{const measured=geometry.partLeft.get(String(part.id||""));return{...part,left:Number.isFinite(measured)?measured:clamp((Number(part.p)+.5)/capacity+(Number(part.x)||0)/width,.005,.995)}})}})}));
