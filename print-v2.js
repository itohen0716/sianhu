(function(global){
  ...
  function captureAnnotations(state){
    ...
    const wraps=staffs.map((staff,row)=>({row,rect:staff.querySelector(".score-wrap").getBoundingClientRect()})),result=new Map();
  ...
  function render(state,options={}){
    ...
    const measureHtml=measure=>`<div class="pv2-measure${measure.endKind!=="single"?" custom-end":""}" style="--grow:${measure.width}">${measure.startKind!=="single"?`<span class="pv2-bar start">${barSvg(measure.startKind)}</span>`:""}${measure.endKind!=="single"?`<span class="pv2-bar end">${barSvg(measure.endKind)}</span>`:""}${measure.parts.map(part=>`<span class="pv2-part" style="left:${(part.left*100).toFixed(5)}%">${sameSvg(part.kind)}</span>`).join("")}${[3,2,1].map(string=>`<div class="pv2-string">${measure.notes.filter(note=>Number(note.s)===string).map(noteHtml).join("")}</div>`).join("")}</div>`;
    root.innerHTML=pages.map(page=>`<section class="pv2-page">${page.index===0?`<div class="pv2-meta"><div class="pv2-meta-left"><span>〈</span><span>${esc(state.tuning||"")}</span><span>〉</span><span>${esc(meterLabel())}</span></div><div class="pv2-title">${esc(state.title||"曲名入力")}</div><div></div></div>`:""}${page.rows.map(row=>`<section class="pv2-staff" style="height:${((283*96/25.4-(page.index===0?34:0))/page.rows.length).toFixed(2)}px"><div class="pv2-wrap"><div class="pv2-labels">${row.row===0?"<span>三の糸</span><span>二の糸</span><span>一の糸</span>":"<span></span><span></span><span></span>"}</div><div class="pv2-score">${row.measures.map(measureHtml).join("")}</div>${row.annotations.length?`<svg class="pv2-overlay" viewBox="0 -60 1000 180" preserveAspectRatio="none">${row.annotations.map(annotationHtml).join("")}</svg>`:""}</div></section>`).join("")}</section>`).join("");
